...
class MotionModel:
    def __init__(self, state0=None, dt = 1e-3, variance=0.0):
        if state0 is None:
            t0, x0, y0, theta0 = 0.0, 0.0, 0.0, 0.0
        else:
            t0, x0, y0, theta0 = state0
...
class SimpleMeasurementModel():
    ...
    # def landmark_transform(self, subject_nb):
    ...
    #     s = np.array([x - self.x, y - self.y])
    #     range = np.linalg.norm(s)

    #     theta_landmark = np.atan2(s[1], s[0])

    #     heading = theta_landmark - self.theta
...
class ParticleFilter:
    ...
    def plot_error(self, h, prefix):
        gt = self.dataloader.data["Groundtruth"]
    ...
    def plot(self, prefix):
        ...
        self.plot_error(avg_history, prefix)
...
def plotbirdseye(prefix, label, x, y, theta, fig2=None, nb_pts = 20, width = 0.001):
    if fig2 is None:
        fig2 = plt.figure()
        c1 = "purple"
        c2 = "orange"
...
def q6(dataloader):
    ...
    for case in cases:
        ...
        range, heading = mm.landmark_transform(id)
