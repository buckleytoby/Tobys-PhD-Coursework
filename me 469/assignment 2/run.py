...
class Logger:
    ...
    def plot_avg(self, key, n):
        ...
        d2 = np.zeros([n] + list(d1.shape[1:]))
        
        d3 = np.concatenate([d2, d1, d2], axis=0)
...
class Training:
    ...
    def ep_summer(self, dd):
        for key, val in dd.items():
            v = np.sum(val)
            
            print("{} total: {}".format(key, v))
            logger.log_one("total_{}".format(key), v)
...
def main():
    ...
    logger.plot_avg("qval", n)
    logger.plot_avg("total_reward", 10)
