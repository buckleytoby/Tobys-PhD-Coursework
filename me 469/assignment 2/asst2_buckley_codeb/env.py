...
def plotbirdseye(prefix, label, x, y, theta, fig2=None, nb_pts = 20, width = 0.005):
    ...
    dx = .5 * np.cos(theta2)
    dy = .5 * np.sin(theta2)
    plt.plot(x2, y2, color=c1, marker='o', linestyle='-', zorder=3, label=label) # ref: https://matplotlib.org/stable/api/_as_gen/matplotlib.pyplot.plot.html

    for i in range(0, dx.shape[0]): # decimate?
        plt.arrow(x2[i], y2[i], dx[i], dy[i], color=c2, zorder=2, width=width/2)
...
class Cell:
    ...
    def plot(self, f):
        ...
        circle = patches.Circle(tuple(self.center), self.width / 2.5, fill=True, color='gold', label="Goal")
        ax.add_patch(circle)
    ...
    def plot(self, f=None):
        ...
        for key, cell in self.cells.items():
            if cell.occupied and not cell.unexplored:
                ...
                rect = patches.Rectangle((x, y), self.cell_width, self.cell_width, fill=True, color='g', label=label)
                ax.add_patch(rect)
...
NNN = 0.0 # no noise for now
class Robot:
    def __init__(self,
                 mm: MotionModel = LinearMotionModel(state0=[0.0, 0.0, 0.0, -np.pi/2.0]),
                 max_accel = 0.288,
                 max_alpha = 5.579,
                 dt = 0.1,
                 v0 = 0.0,
    ...
    def reset(self, node):
        ...
        self.mm = LinearMotionModel(state0=state)
...
def get_range(cell_width = 1.0):
    ...
    nx = int(np.ceil((xlim[1] - xlim[0]) / cell_width))
    ny = int(np.ceil((ylim[1] - ylim[0]) / cell_width))
    # xrange = np.linspace(xlim[0], xlim[1], nx)
    # yrange = np.linspace(ylim[0], ylim[1], ny)
    xrange = np.arange(xlim[0], xlim[1], cell_width)
    yrange = np.arange(ylim[0], ylim[1], cell_width)
